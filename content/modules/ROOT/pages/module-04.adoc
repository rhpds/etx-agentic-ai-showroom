...
For this lab, we've already deployed the pod that runs the https://github.com/containers/kubernetes-mcp-server[**mcp::openshift**,window=_blank] functions. However, we need to configure LlamaStack to use the MCP tool that interacts with the OpenShift cluster. MCP servers are configured similarly to the websearch tool.
...
The last MCP Server we need to deploy https://github.com/github/github-mcp-server[interacts with GitHub,window=_blank]. First, we need to enable access to our github repo so MCP GitHub tool can interact with it.
...
Add MCP GitHub
The configuration is very similar to what we did for enable MCP OpenShift earlier.
For this lab, we’ve already deployed the pod that runs the mcp::github functions.

Now, we need to edit the LlamaStack ConfigMap to add tool runtime and tool group.
