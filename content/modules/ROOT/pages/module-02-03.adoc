...
. Checkout the list of tools that can be invoked via the OpenShift tools provider.
...
.. Run the following command after replacing `<YOUR_GITHUB_USER>` with your GitHub user ID.
+
[source,bash,options="wrap",role="execute"]
----
git clone https://github.com/<YOUR_GITHUB_USER>/etx-agentic-ai-gitops.git
...
. Check the list of tools that can invoked via the github tool provider.
