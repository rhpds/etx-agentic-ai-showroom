...
== Adding the Kubernetes MCP Server to Llama Stack

For this lab, we've already deployed the pod that runs the https://github.com/containers/kubernetes-mcp-server[**Kubernetes MCP server**,window=_blank] that interacts with our OpenShift cluster. Let's add it to Llama Stack as a new tool provider.
...
.. For the https://github.com/rhpds/etx-agentic-ai-gitops[lab repository,window=_blank] to your personal GitHub account.
+
.GitHub Repo Fork
...
.. In the OpenShift Console, choose project ({etx_agentic_ai_username}.llama-stack) and select `Quick create` -> `Import YAML`
...
. **Add the GitHub tool provider in Llama Stack**
...
. Add the following entry under `tool_groups`. Note that we don't have to edit the tool runtime section since we already added the MCP runtime entry previously.
...
. Check the list of tools that can invoked via the MCP:github tool provider.
...
Great, we were able to have the LLM create a GitHub issue for us through a simple chat interface! We're now ready to move on from interactive prototyping to agent development.
...
- We introduced MCP and unlocked Kubernetes and GitHub tools in Llama Stack using pre-built MCP servers.
