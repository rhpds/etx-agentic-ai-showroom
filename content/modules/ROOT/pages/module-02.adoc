...
As part of OpenShift AI, Llama Stack instances are represented by LlamaStack Distribution custom resources (CR) that defines the Llama Stack server configuration including:
...
Also listed is the user configuration used with this distribution (ConfigMap: llama-stack-config). Let's review this configuration before updating the Tavily search key.

=== LlamaStack ConfigMap

A ConfigMap can be used to store run.yaml configuration for each Llama Stack distribution. Updates to the ConfigMap will restart the Pod to load the new data.
