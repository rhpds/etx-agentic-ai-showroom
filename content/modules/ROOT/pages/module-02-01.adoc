...
----
http://{YOUR_USER_NAME}-llama-stack-service.{YOUR_USER_NAME}-llama-stack.svc.cluster.local:8321
----
+
...
Llama Stack comes with a simple UI called Playground. For this lab, a Playground instance is already deployed for each user within the `userx-llama-stack` namespace. Substitute `x` for your individual user number.
