...
https://llama-stack.readthedocs.io/en/latest/[Llama Stack,window=_blank] is an open-source framework and API layer for building and running generative AI applications. It provides a unified set of APIs for inference, RAG, agents, tools, safety, evals, and telemetry.
...
* programmatically from the IDE and agent service through the Llama Stack Python SDK.
...
. Log in to OpenShift if you are not already logged in
+
[source,bash,options="wrap",role="execute"]
----
