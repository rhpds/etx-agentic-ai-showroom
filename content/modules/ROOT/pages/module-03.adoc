...
NOTE: Ensure to replace the values in brackets with the actual values of your user number and forked repository location (you will need to edit the PipelineRun file before execution)

[source,bash,options="wrap",role="execute"]
----
oc -n user[YOUR_USER_NUMBER]-ai-agent create -f etx-agentic-ai-gitops/lab-resources/agent-service-build-run
----
...
. Create the PipelineRun (edit the command and file with your user number):
+
[source,bash,options="wrap",role="execute"]
----
oc -n user${YOUR_USER_NUMBER}-ai-agent create -f etx-agentic-ai-gitops/lab-resources/demo-pipeline-run
----
