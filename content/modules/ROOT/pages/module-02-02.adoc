...
----
+
. Now, via CLI update the Llama Stack distribution to reference this new secret instead of the old one. You must do this with `oc edit` or `oc patch` and you cannot update in the UI. 
+
...
+
. Restart the playground by deleting its pod (starting with `llama-stack-playground`) in the {etx_agentic_ai_username}-llama-stack namespace. Wait until it's Ready and Running. 
