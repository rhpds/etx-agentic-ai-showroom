...
. Via CLI, now update the Llama Stack distribution to reference this new secret instead of the old one. Replace `tavily-search-key` with
...
NOTE: This will need to be done over cli with `oc edit` or `oc patch`, whichever is your preference.
