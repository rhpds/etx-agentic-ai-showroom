...
So far, we've seen how we can query a given model within the Llama Stack Playground and receive __static information__ based on the model's training data. For many use cases, such as troubleshooting a broken CI pipeline, we would like the model to provide __real-time information__ by accessing external systems such as the web or a CI engine. The technical capability that allows a model to interact with a given system is called a __tool__, and the ability to use tools in order to achieve a given objective is considered a core feature of an __AI agent__.

Let's consider our use case and which tools we may need.

Llama Stack simplifies managing and providing tools for agentic use cases. 

Before we try out tool usage and compare agent types in the LLama Stack Playground, we'll have a closer look at how Llama Stack is configured. 

In order to enable tools and update provider information such as the websearch API key, we'll have a closer look at how Llama Stack is configured.
...
----
+
Notice that the agent first **Re**asons about the question before it **Acts** on its reasoning outcome. This two-step approach is the core principle of **ReAct**.
+
